...
def setup_grpc_client() -> None:
    ...
    tasks_channel = grpc.intercept_channel(grpc.insecure_channel(tasks_connection), interceptor)
    property_channel = grpc.intercept_channel(grpc.insecure_channel(property_connection), interceptor)
...
def create_property(name: str) -> uuid.UUID:
    ...
    response: pb.CreatePropertyResponse = property_stub.CreateProperty(request=request)
    logging.info(f"Created property with id {response.property_id}")
    return response.property_id

def attach_property_value(patient_id: uuid.UUID, property_id: uuid.UUID, value: str):
    ...
    response: pb.AttachPropertyValueResponse = property_value_stub.AttachPropertyValue(request=request)
...
def simulate_st_elevation_process() -> bool:
    ...
    attach_property_value(patient_id, medication_property_id, "clopidogrel, prasugrel, beta blockers") 
    ...
    attach_property_value(patient_id, medication_property_id, "aec inhibitor, statins") 
